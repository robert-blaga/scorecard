...
import NameIdentification from "./NameIdentification";
import JobFunctionSelection from "./RoleSelection";
...
export default function UserIdentification({ onComplete, sessionData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      emailError &&
      !GENERIC_EMAIL_DOMAINS.includes(
        formData.business_email.split("@")[1].toLowerCase()
      )
    ) {
      setErrors((prev) => ({
        ...prev,
    ...
    if (formData.gdprConsent) {
      try {
          ...
          surveys: {
            [sessionData?.scorecardId || "unknown"]: {
              ...
              answers: sessionData?.answers
                ? Object.entries(sessionData.answers).reduce(
                    (acc, [questionId, answer]) => {
                      acc[questionId] = {
                        value: answer,
                        score:
                          sessionData?.results?.scores?.[questionId] || null,
                      };
                      return acc;
                    },
                    {}
        ...
        try {
          ...
          localStorage.setItem(
            "marketing_consent",
            formData.marketingConsent ? "true" : "false"
          );
      ...
      case 4:
        return (
          <DataProcessing
            ...
            onGdprConsentChange={(value) =>
              handleInputChange("gdprConsent", value)
            }
  ...
  return (
    <div className="w-full max-w-2xl bg-white rounded-lg shadow-sm border border-gray-100">
      {renderProgressBar()}
      {renderStep()}
    </div>
  );
