...
export default function Users() {
  ...
  // Helper function to get the primary category data
  const getPrimaryCategoryData = (surveyData, scorecard) => {
    if (
      !surveyData?.overall_results?.interpretation ||
      !scorecard?.basic_scoring?.primaryCategory
    ) {
      return {
        needLevel: "Not Available",
        percentage: "0",
      };
    }
    ...
    const interpretation =
      surveyData.overall_results.interpretation[primaryCategory];

    return {
      needLevel: interpretation?.needLevel || "Not Available",
      percentage: interpretation?.percentage?.toString() || "0",
    };
  };
  ...
  const fetchUsers = async () => {
    try {
      ...
      const processedUsers = data.map((user) => {
        ...
        const { needLevel, percentage } = getPrimaryCategoryData(
          surveyData,
          user.survey_answers?.surveys?.["discover-work"]
        );
  ...
  return (
    <div className="p-4 lg:p-6 xl:p-8 max-w-[2000px] mx-auto">
      <h1 className="text-xl lg:text-2xl xl:text-3xl font-bold text-gray-900 mb-4 lg:mb-6">
        Users
      </h1>
